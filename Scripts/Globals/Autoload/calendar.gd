...
const CALENDAR_DATABASE = Databases.CALENDAR_DATABASE
...
func get_save_data() -> Dictionary:
	return {
		...
		"daily_game_minutes": daily_game_minutes,
		"current_day": current_day,
		"current_week_day_index": current_week_day_index,
		"current_season_index": current_season_index,
		"current_year_number": current_year_number,
	}

func set_save_data(data: Dictionary) -> void:
	total_game_minutes = data.get("total_game_minutes", total_game_minutes)
	daily_game_minutes = data.get("daily_game_minutes", daily_game_minutes)
	current_day = data.get("current_day", current_day)
	current_week_day_index = data.get("current_week_day_index", current_week_day_index)
	current_season_index = data.get("current_season_index", current_season_index)
	current_year_number = data.get("current_year_number", current_year_number)
		
	var elapsed_game_seconds := float(total_game_minutes) * REAL_SECONDS_PER_GAME_MINUTE
	start_real_time = (Time.get_ticks_msec() / 1000.0) - (elapsed_game_seconds / time_scale)
	
	minute_tick.emit(get_formatted_time(daily_game_minutes), daily_game_minutes)
	day_change.emit(current_day)
	week_day_change.emit(CALENDAR_DATABASE.week_days.get("week_day_%s" % current_week_day_index))
	season_change.emit(CALENDAR_DATABASE.seasons.get("season_%s" % current_season_index))
	year_change.emit(current_year_number)
